...
function WalletCard({
  ...
  return (
    ...
    >
      {isActive && (
        <div className="absolute top-3 right-3 flex items-center gap-1 text-[10px] font-bold text-[#7C3AED] uppercase tracking-wider">
      ...
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
            isActive ? "bg-[#7C3AED]/20" : "bg-[#1a1d26]"
          }`}
        >
          <Wallet className={`h-5 w-5 ${isActive ? "text-[#7C3AED]" : "text-[#848e9c]"}`} />
        </div>
          ...
          {editing ? (
            <div className="flex items-center gap-2 mb-1">
              <input
                ...
                onBlur={saveLabel}
                autoFocus
                className="bg-[#0b0e11] border border-[#2a2e39] rounded px-2 py-0.5 text-sm text-white outline-none focus:border-[#7C3AED] w-full"
              />
            </div>
          ) : (
            <div className="flex items-center gap-2 mb-1">
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#2a2e39]/50">
        {!isActive && (
          <button
            onClick={onActivate}
            className="flex-1 py-1.5 text-xs font-medium text-[#7C3AED] hover:bg-[#7C3AED]/10 rounded-lg transition-colors"
          >
            Set Active
          </button>
        )}
    ...
    <form onSubmit={handleSubmit} className="p-4 rounded-xl border border-dashed border-[#2a2e39] bg-[#141620]/50 space-y-3">
      ...
      <div>
        ...
        <input
          ...
          placeholder="0x..."
          className="mt-1 w-full bg-[#0b0e11] border border-[#2a2e39] rounded-lg px-3 py-2 text-sm text-white font-mono outline-none focus:border-[#7C3AED] placeholder:text-[#5a6270]"
        />
      </div>
      ...
      <button
        type="submit"
        className="w-full py-2 rounded-lg bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-sm font-semibold transition-colors"
      >
        Add Wallet
      </button>
    </form>
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-xl border border-[#2a2e39] bg-[#141620] space-y-3">
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            onClick={handleSave}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-sm font-semibold transition-colors"
          >
