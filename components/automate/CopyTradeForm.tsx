  ...
  return (
    <div className="space-y-4 max-w-md">
      ...
      <div>
        ...
        <input value={address} onChange={(e) => setAddress(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B] font-mono" placeholder="0x..." />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Label (optional)</label>
        <input value={label} onChange={(e) => setLabel(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B]" placeholder="Whale #1" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-[#848e9c] mb-1">Size Multiplier</label>
          <input value={multiplier} onChange={(e) => setMultiplier(e.target.value)} type="number" step="0.1" className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B]" placeholder="1.0" />
        ...
        <div>
          ...
          <input value={maxUsd} onChange={(e) => setMaxUsd(e.target.value)} type="number" className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B]" placeholder="1000" />
        </div>
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Allowed Coins (optional, comma-separated)</label>
        <input value={allowedCoins} onChange={(e) => setAllowedCoins(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B]" placeholder="BTC, ETH, SOL (leave empty for all)" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Excluded Coins (optional, comma-separated)</label>
        <input value={excludedCoins} onChange={(e) => setExcludedCoins(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#FF455B]" placeholder="DOGE, SHIB" />
      </div>
