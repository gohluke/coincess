  ...
  return (
    <div className="space-y-4 max-w-md">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {(["long", "short"] as const).map((s) => (
            <button key={s} onClick={() => setSide(s)} className={`py-2 rounded-lg text-sm font-medium transition-colors ${side === s ? (s === "long" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30") : "bg-[#0b0e17] text-[#848e9c]"}`}>
              {s.toUpperCase()}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <input value={trail} onChange={(e) => setTrail(e.target.value)} type="number" step="0.1" className="w-full bg-[#0b0e17] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Activation Price (optional)</label>
        <input value={activation} onChange={(e) => setActivation(e.target.value)} type="number" className="w-full bg-[#0b0e17] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand" placeholder="Start trailing after reaching this price" />
      </div>
      <div>
