  ...
  return (
    <div className="space-y-4 max-w-md">
      <p className="text-xs text-[#848e9c] bg-[#141620] rounded-lg p-3 border border-[#2a2e3e]">
        Find the event on the <a href="/predictions" className="text-[#7C3AED] hover:underline">Predictions</a> page, then copy the event ID and token ID from the URL or market data.
      </p>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Event Title</label>
        <input value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="Fed decision in March?" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Event ID</label>
        <input value={eventId} onChange={(e) => setEventId(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="Polymarket event ID" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Market ID</label>
        <input value={marketId} onChange={(e) => setMarketId(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="Condition ID" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Token ID (outcome token)</label>
        <input value={tokenId} onChange={(e) => setTokenId(e.target.value)} className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="CLOB token ID for Yes/No" />
      </div>
      <div>
      ...
      <div>
        ...
        <input value={triggerPrice} onChange={(e) => setTriggerPrice(e.target.value)} type="number" className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="30" />
      </div>
      <div>
        <label className="block text-xs text-[#848e9c] mb-1">Bet Size (USD)</label>
        <input value={betSize} onChange={(e) => setBetSize(e.target.value)} type="number" className="w-full bg-[#0b0e17] border border-[#2a2e3e] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7C3AED]" placeholder="10" />
      </div>
